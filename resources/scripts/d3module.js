...
function load_data(type) {
    ...
    Promise.all([d3.csv("resources/data/" + type + "_cumulative_stance.csv")]).then(createVisualization)
}
...
let monthly_topics, user_sentiments, attributes, dates, month, topics_svg, topic_color, y_scale, sentiment_svg;
...
function visualizeTopics(monthly_topics, month, attributes, topics_svg, y_scale) {
    // Filter the dataset with current month & topic_id != -1
    const current_month_topics = monthly_topics.filter(d => formatMonths(d.month).toLowerCase().replaceAll(' ', '-') === month && d.topic_id !== -1);
    // Group the topics by topic name
    const grouped_topics = d3.rollup(current_month_topics, v => d3.count(v, d => d.score), d => d.topic);
    ...
    topics_svg.selectAll('.bubble')
        ...
        .attr('r', d => Math.ceil(grouped_topics_count.get(d) * 1.5))
...
function handleMouseOverBubble(d, i) {
    ...
    text_node.append('tspan')
        .attr('x', 120)
        .attr('dy', '1.2em')
        .text('Score: ' + i.cumulative_stance);
    text_node.append('tspan')
        .attr('x', 120)
        .attr('dy', '1.2em')
        .text('User: ' + i.user_screen_name);
    text_node.append('tspan')
        .attr('x', 120)
        .attr('dy', '1.2em')
        .text('State: ' + i.state);
...
function createVisualization(data) {
    ...
    monthly_topics = data[0].map((d) => {
        return {
            'month': new Date('01' + d.created_month.trim()),       // Convert string months to date object
            'topic': d.topic.trim().replace(/^(\d+_)+/g, ''),    // Trim empty space and quotes
            'topic_id': parseInt(d.topic_id),   // Convert topic ID to integer
            'score': parseFloat(d.topic_frequency.trim()).toFixed(2)  // Convert to floating point number
        }
    }).filter(d => d.score > 1 && d.topic_id !== -1);
    //user_screen_name,created_at,text,Country,State,created_month,discourage,topic,topic_id,final_stance,stance_cumulative_sum,topic_frequency

    // Prepare the user sentiments with appropriate type conversion
    user_sentiments = data[0].map((d) => {
        return {
            'month': new Date('01' + d.created_month.trim()),
            'created_at': new Date(d.created_at.trim()),
            'cumulative_stance': parseFloat(d.stance_cumulative_sum.trim()).toFixed(2),
            'user_screen_name': 'U_' + btoa(d.user_screen_name.trim()).replaceAll('=', ''),
            'text': d.text,
            'topic': d.topic.trim().replace(/^(\d+_)+/g, ''),
            'topic_id': parseInt(d.topic_id),   // Convert topic ID to integer
            'state': d.State.trim(),
            'stance': (d.final_stance === 'F' ? 'Favor' : d.final_stance === 'A' ? 'Against' : 'Unrelated')
        }
    }).filter(d => d.topic_id !== -1);
    ...
    const x_axis = topics_svg.append('g')
        .attr('class', 'x axis')
        .attr('transform', 'translate(0, ' + (attributes.height_topic - attributes.axis.x - 150) + ')');

    // Create the y axis

    // Get the extent of the scores from all months
    const g = d3.rollup(monthly_topics, v => d3.count(v, d => d.score), d => d.month, d => d.topic);
    let scores = [];
    g.forEach((value, key) => {
    ...
    y_scale = d3.scaleLinear()
        .range([0, attributes.height_topic - attributes.axis.x - 150])
    ...
    // Y axis
    const cumulative_stance_score_extent = d3.extent(user_sentiments.map(d => parseFloat(d.cumulative_stance))).reverse();
    sentiment_y_scale = d3.scaleLinear()
        .range([0, attributes.height_sentiment - attributes.axis.x])
        .domain([cumulative_stance_score_extent[0] + 10, cumulative_stance_score_extent[1] - 10]);
    ...
    // Change data source based on selection
    {
        const data_source_select = d3.select('#data-source');
        data_source_select.on('change', (d) => {
            let selected_source = d3.select(d.target).property('value').toLowerCase();
            clean_slate();
            load_data(selected_source);
        });
    }
...
function clean_slate() {
    d3.select('#topic-visualization').html('');
    d3.select('#sentiment-visualization').html('');
    d3.select('#users').html('');
    d3.select('#months').html('');
    d3.select('#selected-month-label').html('');
}
